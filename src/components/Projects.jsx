      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        ...
        <div className="flex items-end justify-between mb-14">
          <div>
            ...
            <h2
              className="text-4xl md:text-5xl font-black text-[var(--text)] uppercase leading-none"
            >
              FEATURED &amp;{" "}
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="hidden md:inline-flex items-center gap-2 text-sm text-[var(--text-soft)] hover:text-[var(--accent)] transition-colors font-medium"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PROJECTS.map((project, index) => (
            <div
              ...
              className={`group relative rounded-2xl border bg-gradient-to-br from-[var(--surface)] via-[var(--surface-2)] to-[var(--bg)] p-8 flex flex-col justify-between transition-all duration-300 cursor-pointer overflow-hidden shadow-[0_18px_60px_rgba(6,20,27,0.18)] before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-[var(--text)] before:to-transparent ${
                project.size === 'large' ? 'md:col-span-2 min-h-[360px]' : 'md:col-span-1 min-h-[360px]'
              } ${
                hovered === index
                  ? '-translate-y-1 shadow-[0_24px_72px_rgba(6,20,27,0.25)]'
                  : 'border-[var(--border)]'
              }`}
              style={{ borderColor: hovered === index ? 'var(--border-strong)' : 'var(--border)' }}
            >
              <div className="absolute -right-16 -top-16 h-40 w-40 rounded-full bg-[var(--accent-soft)] blur-3xl transition-opacity duration-300 group-hover:opacity-100 opacity-60" />
              {/* Top row */}
              <div className="relative">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <span className="text-[10px] font-semibold tracking-widest uppercase text-[var(--muted)] block mb-1">
                      {project.category}
                    </span>
                    <span className="text-[10px] font-mono text-[var(--muted)]">{project.year}</span>
                  </div>
                  {/* Arrow icon */}
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
                      hovered === index ? 'bg-[var(--accent)] text-[var(--bg)]' : 'bg-[var(--accent-soft)] text-[var(--muted)]'
                    }`}
                  >
                ...
                <h3 className="text-2xl md:text-3xl font-black text-[var(--text)] tracking-tight leading-tight mb-4">
                  {project.title}
                </h3>
              ...
              <div className="relative mt-8 flex flex-wrap gap-2">
                {project.tags.map((tag, tIdx) => (
                  <span
                    key={tIdx}
                    className="text-[11px] px-2.5 py-1 rounded-md bg-[var(--accent-soft)] text-[var(--text)] border font-mono"
